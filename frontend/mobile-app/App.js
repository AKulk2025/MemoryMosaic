...
export default function App() {
  ...
  const fetchPhotos = async () => {
    try {
      ...
      Alert.alert(
        "Error",
        "Failed to fetch photos from server. Make sure your backend is running."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const uploadPhotoToBackend = async (photo) => {
    try {
      ...
      formData.append(
        "title",
        photo.title || `Photo taken at ${new Date().toLocaleString()}`
      );
    ...
    <NavigationContainer>
      ...
      >
        <Stack.Screen name="Map">
          {(props) => <MapScreen {...props} photos={photos} />}
        </Stack.Screen>
